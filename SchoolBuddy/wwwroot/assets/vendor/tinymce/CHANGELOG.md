...
- Nuget packages for .NET and .NET Core are now configured to copy TinyMCE into `/~/lib/` when TinyMCE is installed into a project. #TINY-8611
...
- Fixed the `template` plugin pre~/Views missing some content styles. #TINY-6115
...
- Fixed so the style pre~/Views do not set color and background color when selected.
...
- Added new live pre~/Views for complex elements like table or lists.
...
- Fixed bug where the color pre~/Views in color input boxes wasn't properly updated.
...
- Added new live pre~/Views of embeded videos. Lets you play the video right inside the editor.
